...
function wireSetup() {
  ...
  $('#btn-voice-test').onclick = () => Speech.speak('สวัสดี ยินดีที่ได้รู้จัก', {
...
function syncSetup() {
  ...
  $('#voice-warn').hidden = Speech.hasThaiVoice() || S.setup.voiceMode === 'off';
}
...
function renderQuestion() {
  ...
  const canSpeak = S.setup.voiceMode !== 'off' && Speech.isSupported();
...
function speakCurrent() {
  ...
  Speech.speak(q.card.th, { voiceURI: S.settings.voiceURI, rate: S.settings.voiceRate })
...
function wireSettings() {
  ...
  $('#voice-select').onchange = (e) => {
    ...
    Speech.speak('สวัสดี', { voiceURI: S.settings.voiceURI, rate: S.settings.voiceRate });
  };
...
function renderVoiceSelect() {
  ...
  sel.disabled = false;
  voices.forEach((v) => {
    const isThai = /^th(-|_|$)/i.test(v.lang);
    sel.appendChild(new Option(`${isThai ? '🇹🇭 ' : ''}${v.name} (${v.lang})`, v.voiceURI));
  });
  sel.value = S.settings.voiceURI || voices[0].voiceURI;
